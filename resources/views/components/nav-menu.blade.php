<div class="flex w-full items-center p-4 bg-white rounded-lg">
    <!-- Изображение профиля -->
    @auth
        @if(auth() -> user() ->getFirstMediaUrl('user_images'))
            <img src="{{ auth()->user()->getFirstMediaUrl('user_images') }}"
                 class="object-cover w-16 h-16 rounded-full border-2 border-gray-300" alt="Профиль">
        @else
    ...
    <div class="flex-1 ml-4 flex flex-col items-center text-gray-800">
        ...
        @else
            ...
            <div class="flex space-x-4 mt-2">
                <a href="{{ route('login') }}"
                   class="px-4 py-2 bg-green-500 text-white rounded-md shadow-md hover:bg-green-600 transition">
                    Вход
                </a>

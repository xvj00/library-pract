...
<main class="container mx-auto mt-8 px-6">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <section class="col-span-2 bg-white rounded-lg shadow-md p-6">
            ...
            <table class="w-full bg-gray-50 shadow rounded-lg border-collapse">
    ...
    {{ $reservation->status === 'booked' ? 'bg-green-500' : ($reservation->status === 'confirmed' ? 'bg-blue-500' : ($reservation->status === 'given' ? 'bg-yellow-500' : 'bg-red-500')) }}">
    @if($reservation->status === 'booked')
                        ...
                        <td class="p-4">
                            @if($reservation->status === 'booked')
                                <form action="{{ route('reservation.user.cancel', $reservation->book->id) }}"
                                      method="POST">
                                    @csrf
                                    <button type="submit"
                                            class="px-4 py-2 bg-green-500 text-white rounded-full hover:bg-green-600 transition duration-200">
                                        Отменить
                                    </button>
                                </form>
                            @else
                                <button class="px-4 py-2 bg-gray-300 text-gray-700 rounded-full cursor-not-allowed"
                                        disabled>

    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <section class="col-span-2 bg-white rounded-lg shadow-md p-6">
            ...
            <div class="mb-6">
                ...
                <p class="text-gray-800"><strong>Дата
                        регистрации:</strong> {{ auth()->user()->created_at->format('d.m.Y') }}</p>
                <p class="text-gray-800">
                    <strong>Роль:</strong> {{ auth()->user()->role === 'admin' ? 'Администратор' : (auth()->user()->role === 'librarian' ? 'Библиотекарь' : 'Пользователь')}}
                </p>
            </div>

            <div class="mb-6 flex items-center">
                <div>
                    <img src="{{ auth()->user()->getFirstMediaUrl('user_images') ?: asset('img/default-avatar.png') }}"
                         class="object-cover w-24 h-24 rounded-full border-2 border-gray-300" alt="Аватар">
                </div>
                <form action="{{ route('profile.imageUpdate') }}" method="POST" enctype="multipart/form-data"
                      class="ml-4">
                    @csrf
                    @method('PATCH')
                    ...
                    <button type="submit" class="mt-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600">
                        Сохранить
                    </button>
                </form>
            </div>
